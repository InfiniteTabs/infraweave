...
pub async fn start_local_infrastructure() -> anyhow::Result<LocalInfra> {
    ...
    let dynamo_host = dynamo_container.get_host().await?;
    let dynamo_port = dynamo_container.get_host_port_ipv4(8000).await?;
    // Force use of localhost for Colima compatibility
    ...
    println!(
        "DEBUG: dynamo_host={}, dynamo_port={}, using localhost",
        dynamo_host, dynamo_port
    );
...
async fn bootstrap_resources() -> anyhow::Result<()> {
    ...
    for (env_var, table_name) in &table_map {
        // Only set env var if not already set, to allow user overrides?
        // NO, the user explicitly asked to "utilize the test-scaffold". The test scaffold uses these names.
        // And we are creating the tables with these names.
        // If the user provided .env has different names, the code will fail if we don't overwrite env vars
        // because the table we created is named "events" but the app would look for "Events-Prod...".
