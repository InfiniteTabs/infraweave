...
async fn query_all<Q: DatabaseQuery>(
    ...
    db.query_container(container, &query).await
}

// Helper to query and return first item or error if not found
async fn query_one<Q: DatabaseQuery>(db: &Q, container: &str, query: Value) -> Result<Value> {
    let response = db.query_container(container, &query).await?;
    let items = response
        .get("Items")
        .and_then(|v| v.as_array())
        .ok_or_else(|| anyhow!("Invalid response from query_container"))?;
...
pub async fn get_change_record_impl<Q: DatabaseQuery>(
    ...
    result.map_err(|_| anyhow!("Change record not found"))
}
