...
fn test_end_to_end_simple() {
    ...
    let foo_node = nodes
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "local_file.foo",
            _ => false,
        })
        .expect("local_file.foo not found");

    match foo_node {
        tofu::OutputNode::Resource { data, .. } => {
            assert_eq!(
    ...
    let bar_node = nodes
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "local_file.bar",
            _ => false,
        })
...
fn test_end_to_end_variables() {
    ...
    let file_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local_file.file",
        _ => false,
    });

    let var_content_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "var.content",
        _ => false,
    });

    let var_filename_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "var.filename",
        _ => false,
    });
...
fn test_end_to_end_outputs() {
    ...
    let out_id_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "output.foo_id",
        _ => false,
    });

    let out_content_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "output.foo_content",
        _ => false,
    });
...
fn test_end_to_end_modules() {
    ...
    let inner_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.my_child.local_file.inner",
        _ => false,
    });

    let outer_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local_file.outer",
        _ => false,
    });

    let group_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Group { id, .. } => id == "module.my_child",
        _ => false,
    });
    ...
    match group_node {
        Some(tofu::OutputNode::Group { id, parent_id, .. }) => {
    ...
    match inner_node.unwrap() {
        tofu::OutputNode::Resource { parent_id, .. } => {
            assert_eq!(
                parent_id.as_deref(),
...
fn test_end_to_end_data_sources() {
    ...
    let data_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "data.local_file.read",
        _ => false,
    });

    let dest_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local_file.destination",
        _ => false,
    });
...
fn test_end_to_end_locals() {
    ...
    let var_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "var.input",
        _ => false,
    });

    let out_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local_file.out",
        _ => false,
    });

    // Check for intermediate locals - SHOULD NOT EXIST
    let intermediate = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id.contains("local.intermediate"),
        _ => false,
    });
...
fn test_end_to_end_complex_modules() {
    ...
    let net_grp = nodes.iter().find(|n| match n {
        tofu::OutputNode::Group { id, .. } => id == "module.network",
        _ => false,
    });
    let app_grp = nodes.iter().find(|n| match n {
        tofu::OutputNode::Group { id, .. } => id == "module.app",
        _ => false,
    });
    let db_grp = nodes.iter().find(|n| match n {
        tofu::OutputNode::Group { id, .. } => id == "module.app.module.db",
        _ => false,
    });
    ...
    match db_grp.unwrap() {
        tofu::OutputNode::Group { parent_id, .. } => {
            assert_eq!(
                parent_id.as_deref(),
                Some("module.app"),
    ...
    let vpc_res = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.network.local_file.vpc",
        _ => false,
    });
    let app_res = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.app.local_file.app_server",
        _ => false,
    });
    let db_res = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.app.module.db.local_file.database",
        _ => false,
    });
    let _root_res = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local_file.root_config",
        _ => false,
    });
    let root_var = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "var.environment",
        _ => false,
    });
    let root_out = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "output.final_config_path",
        _ => false,
    });
...
fn test_end_to_end_state_mode() {
    ...
    let file_node = result
        ...
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "local_file.state_test",
            _ => false,
        })
...
fn test_end_to_end_locals_constant() {
    ...
    let file_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local_file.f",
        _ => false,
    });

    let local_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "local.constant_val",
        _ => false,
    });
...
fn test_end_to_end_module_constant_output() {
    ...
    let out_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.c.output.out",
        _ => false,
    });
...
fn test_end_to_end_module_var_hidden() {
    ...
    let file_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.m.local_file.inner",
        _ => false,
    });

    let var_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.m.var.inner_val",
        _ => false,
    });
...
fn test_end_to_end_count_and_foreach() {
    ...
    let counted = nodes
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "local_file.counted",
            _ => false,
        })
    ...
    let foreach = nodes
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "local_file.foreach",
            _ => false,
        })
...
fn test_mock_empty_module_pruning() {
    ...
    let resource_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.empty_mod.null_resource.placeholder",
        _ => false,
    });
    assert!(resource_node.is_none(), "Disabled resource should be gone");

    let module_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Group { id, .. } => id == "module.empty_mod",
        _ => false,
    });
...
fn test_end_to_end_module_inner_outputs() {
    ...
    let root_out = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "output.root_out",
        _ => false,
    });

    let inner_out = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "module.inner.output.inner_out",
        _ => false,
    });
...
fn test_phantom_data_source_pruning() {
    ...
    let ghost_node = nodes.iter().find(|n| match n {
        tofu::OutputNode::Resource { id, .. } => id == "data.local_file.ghost",
        _ => false,
    });
...
fn test_state_file_mode() {
    ...
    let prod_node = graph
        ...
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "aws_instance.prod",
            _ => false,
        })
    ...
    let data_node = graph
        ...
        .iter()
        .find(|n| match n {
            tofu::OutputNode::Resource { id, .. } => id == "data.aws_ami.ubuntu",
            _ => false,
        })
...
fn test_realistic_eks_module_outputs() {
    ...
    for node in &nodes {
        match node {
            tofu::OutputNode::Resource { id, data, .. } => {
                println!(
                    "  - {} (type: {}, action: {:?})",
                    id, data.node_type, data.action
                );
            }
            tofu::OutputNode::Group { id, .. } => {
                println!("  - {} (GROUP)", id);
            }
        }
    }
    ...
    let node_ids: std::collections::HashSet<String> = nodes
        ...
        .filter_map(|n| match n {
            tofu::OutputNode::Resource { id, .. } => Some(id.clone()),
            tofu::OutputNode::Group { id, .. } => Some(id.clone()),
        })
        .collect();
